...
# The fixed number of vacation days that can ben paid out.
FIXED_VACATIONS_DAYS_PAYOUT = 5
...
class Employee(ABC):
    ...
    def take_a_holiday(self) -> None:
        """Let the employee take a holiday (lazy bastand)."""
        if self.vacation_days < 1:
            raise VacationDaysShortageError(
                requested_days=1,
                remaining_days=self.vacation_days,
                message="You don't have any holiday left. Now back to work, you!",
            )
    ...
    def payout_a_holiday(self) -> None:
        ...
        if self.vacation_days < FIXED_VACATIONS_DAYS_PAYOUT:
            raise VacationDaysShortageError(
                requested_days=FIXED_VACATIONS_DAYS_PAYOUT,
                remaining_days=self.vacation_days,
                message="You don't have enough holidays left over for a payout",
            )
        self.vacation_days -= FIXED_VACATIONS_DAYS_PAYOUT
...
class SalariedEmployee(Employee):
    ...
    def pay(self) -> None:
        print(f"Paying employee {self.name} a monthly salary of "
...
def main() -> None:
    ...
    company.add_employee(SalariedEmployee(name="Carlos José", role=Role.MANAGER))
    company.add_employee(HourlyEmployee(name="Marcos Rocha", role=Role.PRESIDENT))
    company.add_employee(HourlyEmployee(name="Maria Aparecida", role=Role.INTERN))
