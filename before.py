...
# The fixed number of vacations days that can be paid out.
    ...
    def take_a_holiday(self, payout: bool) -> None:
        ...
        if payout:
            ...
            try:
                ...
                print(f"Paying out a holiday. Holydays left: {self.vacation_days}")
            except Exception:
...
class Company:
    ...
    def find_vice_presidents(self) -> List[Employee]:
        """Find all vice_presidents employees"""
        vice_presidents = []
        for employee in self.employees:
...
def main() -> None:
    ...
    company.add_employee(SalariedEmployee(name="Fábio Azevedo", role="manager"))
    company.add_employee(HourlyEmployee(name="Carlos Azevedo", role="president"))
    company.add_employee(HourlyEmployee(name="Hernandes Dias", role="intern"))
